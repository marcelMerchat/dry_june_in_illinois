...
## (1) PointLocationVector Line-55
## (2) VelocityVector      Line-251
## (3) ForceVector         Line-437
## (4) Planet              Line-557
## (5) SolarSystem         Line-773 
## (6) CenterOfMass        Line-987
## (7) OrbitData           Line-1037 
## (8) OrbitAnalysis       Line-1464
## (9) OrbitController     Line-1956

## Generate Simulated Data for Solar System Orbits               Line-2345
## The orbit data is saved as a csv file.

## Function-1: run_orbit_analyzer                                Line-2404

## Examples of running code:                                     Line-2446

#############################################################################

## Process raw simulated orbit data to make irradiation year records

## Class Definitions

## (10) DailyOrbitSummary   Line-2486
## (11) YearOrbitSummary    Line-2582

#############################################################################

## Combine Solar Irradiation with Weather and Lake-Level Data    Line-2675       
## This makes merged meterological csv data file.

## Class Definition

## (12) DataProcessor                                            Line-2716

## Example of script command to declare 'DataProcessor' object:  Line-2826
## The final merged meteorological file is generated by this.  
    ...
    list(
       ...
       initialize = function(...,degrees=TRUE) {
         ...
         if(x_found > 0 && y_found > 0){
            ...
            self$theta <- theta
            self$phi <- phi
            #cat("Vector position of magnitude = ", mag," with direction of " ,theta, " degrees.\n")
         }
       },
       set_x = function(x) {
           self$x <- x
...
CenterOfMass <- R6Class("CenterOfMass", 
  list(
    ...
    z = NA,
    mass = NA,
    # Error in self$z <- z : cannot add bindings to a locked environment
    initialize = function(planet1, planet2) {
      ...
      m <- m1 + m2
      self$mass <- m
      #cat("Center of" ,m,"kg mass at x  = ",x,"meters, y = ",y,"meters\n")
    },
...
OrbitData <- R6Class("OrbitData", 
    list(
      ...
      get_orbit_data = function(solar_system, planet,planet_name,v0,
        ...
        # browser()
        df <- data.frame(time,cmx,cmy,cmz,
        ...
        rows <- dim(df)[1]  
        ##lagintervals <- ceiling(lagintervals/decimator)
        ## new <- df[2:rows,] 
        new <- df
        #rows2 <- dim(new)[1]
        # orginaltimes <- new$time
        # oldstart <- new$time[1]
        # oldvec <- rep(oldstart,rows2)
        # timevec <- orginaltimes - oldvec
        # new[,1] <- timevec
        new
     },
     get_cm = function(planet1,planet2){
  ...
  list(
    ...
    initialize = function(planet, df, theta0=0, optimized=FALSE) {
      ...
      if(optimized==TRUE) {
        ...
        #self$aphelion_corr <- self$get_aphelion_correction(df)
      }
      # browser()
      #self$get_aphelion_correction(df)
    },
    ...
    get_aphelion_correction = function(df){
      ...
      ##aphelion_error <- diff(aphelion_time, aphelion_1959)
      ...
      # jan_01_1959_data <- as.POSIXct(dfanys[1,"time"], origin="1970-01-01")
      # jan_01_1959_data_secs <- as.numeric(jan_01_1959)
      # 
      # year_event_time <- diff(aphelion_1959, jan_01_1959)
      # data_event_time <- diff(aphelion_1959, jan_01_1959) 
      # 
    ...
    get_irradiation_plot = function(){
      ...
      total <- area * watt_m2  # [1] 3.829803e+26 384.6 yotta watts (3.846×1026 watts)  https://en.wikipedia.org/wiki/Solar_irradiance
    ...
    list(
      ...
      if (optimized==TRUE && loops > 0){
         for(i in 1:loops) {
            ...
            planet$set_v0(v0)
            printv0 <- formatC(v0, digits = 4, format = "f")
            #cat("Initial velocity = ",printv0,"m/s, initial_heading = ", planet$heading0,"degrees\n")
            cat("Initial velocity for ",planet0," = ",printv0,"m/s, initial_heading = ",
                planet$heading0,"degrees\n")
            #cat("Initial velocity for ",planet1_name," = ",print1v0,"m/s, initial_heading = ",
      ...
      get_orbit_summary = function(df, planet_year, theta0,theta1,
        ...
        ###########################################################
        ##planet_year <-   4332.589 *  3600 * 24  # 365.256
        ## yr 31558119
        ...
        if (record_minus1 < (rows - 3)) {
          ...
          # Orbit <- c(seconds_yr/yr,degrees_year)
          if(degrees_year1 < 180){
            degrees_year1 <- degrees_year1 + 360
          } 
          angular_speed1 <- degrees_year1
      ...
      get_jupiter_start = function(df){
        ...
        for(i in 1:rows_logic){
          ...
          #jup_lag_logic[j] <-  dfanys$planet1_angle[i] < lag0  &&  dfanys$planet1_angle[j] > lag0
...
## Examples of runnning the function named 'run_orbit_analyzer' 
## to generate a csv file of simulation results. By setting optimized to True
## and loops to 2 or more, the simulation will be repeated with a corrected 
## initial velocity to obtaining an orbital period closer to one year.
## When the orbital period is considered close enough to one year, the 
## simulation can be run with optimized=FALSE and loops=1.
## Setting revolutions=12 will simulate the earth orbit over one period of
## Jupiter's period. Setting revolutions=72 will simulate the earth and Jupiter
...
run_orbit_analyzer <- function(planet="earth",sample_rate=6000,
  ...
  if(optimized == TRUE){
    ...
    write.csv(df,
              ...
              row.names = FALSE)
    #list(analyzer$data, analyzer$period, analyzer$v0, analyzer$aphelion)
...
# result <- run_orbit_analyzer(planet="jupiter", sample_rate=48000,v0_earth=29303,
...
DailyOrbitSummary <- R6Class("DailyOrbitSummary", 
    list(
        ...
        generate_daily_irradiation_records = function(sample_rate=8000, 
          ...
          daily_solar_data <- daily_solar_data[,c(34,1:33,35)] 
